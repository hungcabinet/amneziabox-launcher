...
### Код — это обязательство, а не актив
Каждая строка кода — это строка, которую надо читать, понимать, поддерживать и отлаживать. Лучший код — тот, который не написан. Перед добавлением нового кода спроси себя: можно ли решить задачу удалением или упрощением существующего?

### Простота важнее абстракции
Не создавай абстракции «на будущее». Интерфейс нужен, когда есть ≥2 реализации или реальная потребность в моках для тестов. Адаптер нужен, когда типы действительно несовместимы. Обёртка нужна, когда она добавляет логику. Если обёртка только пробрасывает вызов — это шум, а не архитектура. **Лишний слой абстракции хуже, чем его отсутствие.**

### Единственный источник правды
Данные должны храниться в одном месте. Если определение существует в шаблоне — не дублируй его в состоянии. Если константа определена в пакете — не объявляй такую же в другом. Дублирование неизбежно приводит к рассинхронизации.

### Обратная совместимость — функциональная, не API
Пользователь не видит сигнатуры функций — он видит поведение. Сохраняй **что** программа делает, но свободно меняй **как**. Старые типы, интерфейсы, функции, которые мешают чистой реализации — удаляй. Вызывающий код, который ломается — исправляй, а не оборачивай.

### Удаление кода — тоже результат
Рефакторинг, при котором удаляется мёртвый код, упрощаются обёртки, убираются ненужные абстракции — это не «ничего не сделал», это улучшение. Не бойся удалять.

### Думай данными, а не кодом
Прежде чем писать функцию, определи структуру данных. Правильная структура данных делает код очевидным. Неправильная — порождает костыли, обёртки и «особые случаи».

### Идиоматичный Go
- Конкретные типы, пока не доказана необходимость интерфейса
- Композиция вместо наследования
- Ошибки как значения, а не исключения
- Горутины + каналы для конкурентности, мьютексы — когда каналы избыточны
- `defer` для всего, что требует закрытия

---

## Язык комментариев

- Комментарии допустимы **на русском и английском** языке
- Go-стиль сохраняется: `// FunctionName описание функции`
- Пакетные комментарии: `// Package xxx описание пакета`
- Длинные пояснения бизнес-логики допустимы на русском
- В рамках одного файла рекомендуется придерживаться одного языка комментариев
...
- **НЕ** создавай обёрток без реальной логики — адаптеры, прокси, фасады допустимы только если добавляют поведение или нужны для тестирования. Обёртка, которая только пробрасывает вызов — лишний слой
- **НЕ** создавай интерфейс ради одной реализации — используй конкретный тип, интерфейс появится когда будет вторая реализация или потребность в моках
...
- Если можно вызвать функцию напрямую — не оборачивай её в другую функцию

### 4. Комментирование
- Комментарии допустимы **на русском и английском** (см. раздел «Язык комментариев» выше)
- Комментируй **почему**, а не **что** (код должен быть самодокументирующимся)
- Используй Go-стиль: `// FunctionName описание функции` перед функциями
- Пакетные комментарии: `// Package xxx описание пакета`
...
- Обрабатывай ситуации, когда данные могут быть некорректными
...
- Проверяй существование файлов и директорий перед работой с ними (`os.Stat`)
...
- Обратная совместимость — **функциональная**: поведение программы для пользователя не должно ломаться, но внутренние API (сигнатуры, типы, интерфейсы) можно и нужно менять, если ТЗ этого требует
- Если старый код мешает чистой реализации — удаляй его и исправляй вызывающий код, а не создавай обёртку
- Если нужно нарушить пользовательскую совместимость (форматы, CLI, конфиги) - явно укажи это в выводе и обоснуй
...
### 17. Работа с Fyne GUI
- **Все обновления UI из горутин должны выполняться через `fyne.Do`** - это гарантирует выполнение в главном потоке Fyne
- **Используй существующие обертки** - если в проекте есть `SafeFyneDo` или `UpdateUI`, используй их вместо прямого `fyne.Do`
- **Избегай вложенных `fyne.Do`** - если код уже выполняется внутри `fyne.Do` (например, в обработчике кнопки или внутри `UpdateUI`), не оборачивай операции в дополнительный `fyne.Do`:
  ```go
  // ❌ ПЛОХО: вложенный fyne.Do
  p.UpdateUI(func() {
      button := widget.NewButton("OK", func() {
          fyne.Do(func() {  // ← избыточный fyne.Do!
              window.Close()
          })
      })
  })
  
  // ✅ ХОРОШО: прямой вызов, т.к. обработчик уже в главном потоке
  p.UpdateUI(func() {
      button := widget.NewButton("OK", func() {
          window.Close()  // ← прямой вызов безопасен
      })
  })
  ```
- **Проблемы вложенных `fyne.Do`**:
  - Могут вызывать проблемы с контекстом выполнения и порядком событий
  - Могут приводить к неожиданному поведению (например, закрытие не того окна)
  - Создают избыточные уровни отложенного выполнения
- **Когда использовать `fyne.Do`**:
  - При обновлении UI из горутины (не из главного потока)
  - При вызове из асинхронных операций (например, после `go func()`)
- **Когда НЕ использовать `fyne.Do`**:
  - В обработчиках событий (кнопки, поля ввода) - они уже выполняются в главном потоке
  - Внутри функций, уже обернутых в `fyne.Do` или `UpdateUI`
  - При прямых вызовах из главного потока приложения

### 18. Обработка неполного ТЗ
...
1. **Простота важнее абстракции** — не создавай слои «на будущее», решай сегодняшнюю задачу
2. **Приоритет читаемости** — код должен быть понятным для других разработчиков
3. **Минимальный дифф** — меняй только то, что нужно для задачи, без самовольных улучшений
4. **Удаление — тоже результат** — убирай мёртвый код, лишние обёртки, ненужные абстракции
5. **Не дублируй данные** — единственный источник правды для каждого факта
6. **Не дублируй логику** — выноси общую функциональность в отдельные функции
7. **Не оборачивай без причины** — если можно вызвать напрямую, вызывай напрямую
8. **Используй существующие паттерны** — следуй архитектуре и стилю проекта
9. **Проверяй существующие утилиты** — перед написанием новой функции изучи доступные пакеты
10. **Управляй ресурсами** — всегда используй `defer` для освобождения ресурсов
11. **Устанавливай таймауты** — для всех сетевых и долгих операций
12. **Безопасность логов** — никогда не логируй секреты и чувствительные данные
13. **Оборачивай ошибки** — используй `%w` для сохранения первопричины
14. **Комментарии** — допустимы на русском и английском; в рамках файла рекомендуется единый язык
